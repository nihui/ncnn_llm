...
int main() {
    ...
    ncnn_llm_gpt model("./assets/hunyuan_0.5b",
                       /*use_vulkan=*/false);

    std::cout << "Chat with Hunyuan-0.5B! Type 'exit' or 'quit' to end the conversation.\n";

    std::string prompt = apply_chat_template({
        {"system", "You are a helpful assistant."},
    }, {}, false, false, TemplateType::HUNYUAN);
    ...
    while (true) {
        ...
        std::string user_message = apply_chat_template({
            {"user", input}
        }, {}, true, true, TemplateType::HUNYUAN);
